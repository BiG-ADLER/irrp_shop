...
function openInventoryShop(shopNumber)
    ESX.TriggerServerCallback('irrp_shop:getinventory', function(inventory)
        ...
        if inventory then
                   ...
                   if data3.value then
                    ...
                    if count then
                        if count > 0 then
                            ...
                            ESX.ShowNotification("~h~Number should more than 0")
                        end
                    else
                        ESX.ShowNotification("~h~You should just put number here")
                    end
                    
                       
                   else
                    ESX.ShowNotification("~h~You should just put number")
                   end
            ...
            ESX.ShowNotification("~h~Some thing went wrong!")
        end
       ...
       if data3.value then
        ...
        if count then
            if count > 0 then
                ...
                ESX.ShowNotification("~h~Price should more than 0")
            end
        else
            ESX.ShowNotification("~h~Just enter some number")
        end
        
           
       else
        ESX.ShowNotification("~h~You didnt put anything!")
       end
...
function openChangeNameMenu(shopNumber)
    ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'shop_changename', {
        ...
        if not data3.value then
            ESX.ShowNotification("Minimum char : ~g~3")
            return
        end
        ...
        if string.len(trim1(data3.value)) >= 3 then
            ...
            ESX.ShowNotification("~h~Minimum char : ~g~3")
        end
    end, function (data3, menu3)
        menu3.close()
        HasAlreadyEnteredMarker = false
...
--                            ExecuteCommand("closei")
                            ...
                            doing = false
                            ESX.ShowNotification('Ebteda roye forooshande aim begirid!')
                        end
                    end, shopId)
                    while doing do
                        Wait(1000)
                ...
                if isRegistered then
                    ...
                    if returnValue == 1 and GetWeaponDamageType(currentPedWeapon) == 3 and HasEntityClearLosToEntity(PlayerPedId(), entity, 17) then
                        ...
                        elseif canRob == 'no_cops' then
                            ESX.ShowNotification('Dar hale hazer tedad morede niaz police online nist!')
                            Citizen.Wait(10000)
                        else
                            ESX.ShowNotification('In shop be tazegi behesh dastbord zade shode, lotfan kami sabr konid!')
                            Citizen.Wait(10000)
                        end
                    end
                end
